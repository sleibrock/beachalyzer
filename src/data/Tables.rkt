...
(define (get-levels name table)
  (if (empty? table)
    ...
    (if (string=? name (building-name (first table)))
...
(module+ test
  ...
  (displayln "Testing correct number of level records")
  (for-each
   (λ (building)
     (define name (building-name building))
     (define kv   (assoc name max-levels))
     (check-not-eq? kv #f
                    (format "Building ~a  not found in max levels list"
                            (building-name building)))
     (check-eq? (cdr kv) (length (building-table building))
                (format "Building ~a has invalid number of records"
                        (building-name building))))
   all-buildings)
  )
